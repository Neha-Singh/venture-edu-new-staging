        ...
        features: [
           {
                ...
                logo: ["/about/advisory/vistara.svg","/about/advisory/tata_realty.svg"]
            }, {
    ...
    return (<>
        <div className="md:py-12 py-4  bg-white w-full">
            <div className="flex flex-col items-center justify-center gap-2 md:mb-8 mb-4">
                <h2 className="text-2xl md:text-4xl font-bold text-zinc-700">Advisory Panel</h2>
                <p className="text-center"> Experts from private markets, credit & financial services</p>
            </div>
                    ...
                    <CarouselContent>
                        {
                            advisoryPanelData?.features?.map((feature, index) => (
                                <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/4 lg:mx-6 lg:max-w-[25%] h-[auto] max-w-[45%] p-2 ">
                                    <div className=" flex flex-col items-start justify-center gap-0  shadow-md rounded-md relative">
                                        <Link href={feature.link} target="_blank" rel="noopener noreferrer" className="z-40  hover:shadow-xl hover:scale-115 h-[24px] w-[24px] bg-[url(/about/linkedin.svg)] bg-contain" style={{ position: "absolute", right: "8px", top: "8px", cursor: "pointer" }}/>
                                        ...
                                        <div className=" w-full py-2 px-3 bg-blue-50">
                                            <div className="text-zinc-600 font-bold text-sm md:text-2xl">{feature.name}</div>
                                            <div className="flex items-center justify-start mt-1  w-full">
                                            {feature.logo?.length ? feature.logo.map((logo, idx) => (
                                                <div key={idx} className="max-h-[30px] max-w-[50%] whitespace-nowrap text-blue-800 pr-1.5 py-1.5 rounded-md mr-2 mb-2 font-medium  ">
                                                    <Image
